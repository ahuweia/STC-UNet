...
def b16(n, activation, resolution=224):
    return torch.nn.Sequential(
        Conv2d_BN(3, n // 8, 3, 2, 1, resolution=resolution),  # aping num_channel
        activation(),
...
class LeViT_UNet_192(BaseDecodeHead):
    ...
    def __init__(self, img_size=224,  ## aping: should change
                 ...
                 drop_path=0,
                 **kwargs):
        super(LeViT_UNet_192, self).__init__(**kwargs)
        global FLOPS_COUNTER
    ...
    def forward(self, x):
        ...
        if (x_r_1.shape != x_r_2_up.shape):
            x_r_2_up = F.interpolate(x_r_2_up, size=x_r_1.shape[2:], mode="bilinear", align_corners=True)
